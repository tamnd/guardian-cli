// Package cli assembles the guardian command tree from the guardian
...
// NewApp assembles the kit application from the guardian domain. The
...
//
// To add a command, declare it in guardian/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := guardian.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(guardian.Domain{}).Register(app)
