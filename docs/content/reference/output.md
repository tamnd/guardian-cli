...
describes what users get. Pick a format with `-o`, or let guardian choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
guardian <command> -o table   # aligned columns for reading
guardian <command> -o jsonl   # one JSON object per line, for piping
guardian <command> -o json    # a single JSON array
guardian <command> -o csv     # spreadsheet friendly
guardian <command> -o tsv     # tab-separated
guardian <command> -o url     # just the URL column
guardian <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
guardian <command> --fields id,title,url
```
...
```bash
guardian <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
guardian <command>            # a table, because this is a terminal
guardian <command> | wc -l    # JSONL, because this is a pipe
```
